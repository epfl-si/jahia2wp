...
class WPExporter:
    ...
    def __init__(self, site, wp_generator, default_language, output_dir=None):
        ...
        # To store asciify history. Key=original, value=renamed
        self.asciify_history = {}
    ...
    def __asciify_string(self, string):
        """
        Transform string to ascii-only characters. Stores changed strings to be able to handle duplicates. If a "target"
        string has already been generated, we ensure a different one is generated by adding a random string as suffix.

        :param string: string to asciify
        :return:
        """

        # If we already have a new name for the string, we use it
        if string in self.asciify_history:
            return self.asciify_history[string]

        # Change "indu socie?\udcef\udcbf\udcbd an.jpg" to "indu socie???? an.jpg/indu socie???? an.jpg"
        new_string = string.encode('ascii', 'replace').decode('ascii')

        # If new name is already used
        if new_string in self.asciify_history.values():
            # we have to make it unique. We just add a random string at beginning.
            new_string = "{}{}".format(WPUtils.generate_random_b64(6), new_string)

        # Saving for futur use
        self.asciify_history[string] = new_string
    ...
    def _asciify_path(self, path):
        ...
        for file_name in files:
            try:
                ...
                ascii_file_name = self.__asciify_string(file_name)
        ...
        for dir_name in dirs:
            try:
                ...
                ascii_dir_name = self.__asciify_string(dir_name)
    ...
    def escape_menu_entry_txt(self, menu_txt):
        """
        Escape special characters in menu entry text

        :param menu_txt: text to escape
        :return:
        """
        return menu_txt.replace('"', '\\"').replace("`", '\\`')
            ...
            if not menu_item.hidden:
                ...
                if menu_item.points_to_url() or menu_item.points_to_sitemap():
                    ...
                    cmd = 'menu item add-custom {} "{}" "{}" {} --parent-id={} --porcelain' \
                        .format(menu_name, self.escape_menu_entry_txt(menu_item.txt), url, target, parent_menu_id)
                ...
                # menu entry is page
                    ...
                    target_page = self.site.homepage.get_page_with_uuid(menu_item.points_to, 4)
                    ...
                    if lang in target_page.contents and target_page.contents[lang].wp_id:

                        # If we have a menu entry title and it is different as the page title, we take the menu title
                        menu_txt = menu_item.txt if menu_item.txt != "" else target_page.contents[lang].menu_title
                        ...
                        if not menu_id:
                            logging.warning("Menu not created for page %s", target_page.pid)
                        else:
                ...
                for root_entry_index, menu_item in enumerate(self.site.menus[lang]):
                    ...
                    if not menu_item.hidden:
                        ...
                        if menu_item.points_to_url() or \
                            ...
                            cmd = 'menu item add-custom {} "{}" "{}" {} --porcelain'.format(
                                menu_name, self.escape_menu_entry_txt(menu_item.txt), url, target)
