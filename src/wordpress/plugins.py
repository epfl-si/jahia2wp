...
class WPPluginList:
    ...
    def __init__(self, generic_config_path, generic_plugin_yaml, specific_config_path):
        ...
        for plugin_infos in plugin_list['plugins']:
            ...
            self._generic_plugins[plugin_infos['name']] = WPPluginConfigInfos(plugin_infos['name'], plugin_infos['config'])
    ...
    def __build_plugins_for_site(self, site_name):
        ...
        if 'plugins' not in plugin_list:
            logging.error("%s - YAML format error. 'plugins' key not found in file: %s", repr(self), site_specific_plugin_file)
        ...
        for plugin_infos in plugin_list['plugins']:
            ...
            # We don't already have plugin configuration
                ...
                specific_plugins[plugin_infos['name']] = WPPluginConfigInfos(plugin_infos['name'], plugin_infos['config'])
