...
then
	...
	wp option update seed_csp4_settings_content --format=json --path=/srv/www/www.epfl.ch/htdocs/${dest} < /tmp/activation_coming-soon_archive_${site}
	rm /tmp/activation_coming-soon_archive_${site}

	#Mettre les configurations du plugin coming-soon
	python ../jahia2wp.py update-plugins www https://www.epfl.ch/${dest} --plugin=coming-soon --extra-config=/srv/www/jahia2wp/functional_tests/extra.yaml --force-options
